frappe.ui.form.on('Student Group', {
  ...
  refresh: function (frm) {
	...
	// frappe.call({
	// 	method: "get_instructors_in_group",
	// 	freeze: 1,
  ...
  get_seperated_members: function (frm){
		...
		frappe.call({
			...
			callback: function (response) {
				...
				// Tạo bảng HTML
				let erpnext_only_instructors_table_html = `
					<table class="table table-bordered">
						<thead>
							<tr>
								<th>Instructor ID(Doc Name)</th>
								<th>Name</th>
								<th>Email</th>
								<th>Actions</th>
							</tr>
				...
				// Đổ dữ liệu vào bảng
				data.only_in_erpnext_instructors.forEach(instructor => {
					erpnext_only_instructors_table_html += `
						<tr>
							<td><a href="/app/instructor/${instructor.name}">${instructor.name}</td>
							<td>${instructor.instructor_name}</td>
							<td>${instructor.user_id}</td>
							<td><button onclick = "promote_member_to_m365_admin('${instructor.user_id}')" class = "btn btn-default">Promote to Administrator</button>
							<br><br><button onclick = "unlink_from_student_group('${instructor.name}')" class = "btn btn-default">Unlink from this Student Group</button>					
						</tr>
					`;
					// <br><br><button onclick = "add_member_to_m365_via_power_automate('${student.name}')" class = "btn btn-primary">Add to M365 via Power Automate</button></td>
				});
				...
				// Đưa bảng HTML vào trường HTML Field
				frm.fields_dict['erpnext_only_instructors_table_html'].$wrapper.html(erpnext_only_instructors_table_html);

				let m365_only_admins_html_table = `
					<table class="table table-bordered">
						<thead>
							<tr>
								<th>Office 365 ID</th>
				...
				// Đổ dữ liệu vào bảng
				data.only_in_m365_admins.forEach(member => {
					m365_only_admins_html_table += `
						<tr>
							<td>${member.id}</td>
							<td>${member.displayName}</td>
							<td>${member.mail}</td>
				...
				// Đưa bảng HTML vào trường HTML Field
				frm.fields_dict['m365_only_admins_html_table'].$wrapper.html(m365_only_admins_html_table);

				let both_admins_table_html = `
					<table class="table table-bordered">
						<thead>
							<tr>
								<th>Instructor ID(Doc Name)</th>
				...
				// Đổ dữ liệu vào bảng
				data.both_admins.forEach(instructor => {
					both_admins_table_html += `
						<tr>
							<td><a href="/app/instructor/${instructor.name}">${instructor.name}</td>
							<td>${instructor.office_365_id}</td>
							<td>${instructor.instructor_name}</td>
							<td>${instructor.office_365_name}</td>
							<td>${instructor.user_id}</td>
							<td><button onclick = "unlink_from_student_group('${instructor.name}')" class = "btn btn-default">Unlink from this Student Group</button>
							<br><br><button onclick = "remove_member_from_m365('${instructor.user_id}')" class = "btn btn-default">Remove from M365 Group</button></td>
						</tr>
					`;
				});
				...
				// Đưa bảng HTML vào trường HTML Field
				frm.fields_dict['both_admins_table_html'].$wrapper.html(both_admins_table_html);
			}
		});
	}
})
