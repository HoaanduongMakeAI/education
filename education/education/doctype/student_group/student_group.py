...
class StudentGroup(Document):
	...
	def get_m365_admins_on_server(self):
		if self.m365_group:
			m365_group = frappe.get_doc("M365 Groups",self.m365_group)
			return m365_group.get_m365_admins_on_server()
		else:
			return []
	...
	def get_seperated_members(self):
		...
		erpnext_instructors = self.get_instructors_in_group()

		m365_admins = self.get_m365_admins_on_server()

		instructor_mails = {member["user_id"] for member in erpnext_instructors}

		m365_admin_lookup = {member["mail"]: member for member in m365_admins}

		# Lọc ra các thành viên chỉ có trong ERPNext
		only_in_erpnext_instructors = [member for member in erpnext_instructors if member["user_id"] not in m365_admin_lookup]

		# Lọc ra các thành viên chỉ có trong M365
		only_in_m365_admins = [member for member in m365_admins if member["mail"] not in instructor_mails]

		both_admins = [
			{
				**erp_member,  # Toàn bộ thuộc tính từ erpnext_members
				"office_365_id": m365_admin_lookup[erp_member["user_id"]]["id"],  # Thuộc tính mới
				"office_365_name": m365_admin_lookup[erp_member["user_id"]]["displayName"],  # Thuộc tính mới
			}
			for erp_member in erpnext_instructors
			if erp_member["user_id"] in m365_admin_lookup  # Chỉ thêm nếu có trong m365_lookup
		]
		...
		"only_in_erpnext_instructors":only_in_erpnext_instructors,"only_in_m365_admins":only_in_m365_admins,"both_admins":both_admins}
	...
	def promote_member_to_m365_admin(self,email):
		if(self.m365_group):
			m365_group = frappe.get_doc("M365 Groups",self.m365_group)
			return m365_group.promote_member_to_m365_admin(email=email)
		else:
			frappe.throw("This Department doesn't have a M365 Group. Please create M365 Group for This Department.")
	...
	@frappe.whitelist()
	def get_instructors_in_group(self):
		instructors_list = []
		for s in self.instructors:
			instructor = frappe.get_doc("Instructor", s.instructor).as_dict()
			instructor["employee_id"] = instructor["employee"]
			instructor["employee"] = frappe.get_doc("Employee", instructor["employee_id"]).as_dict()
			instructor["user_id"] = instructor["employee"]["user_id"]
...
def convert_to_identifier(input_str):
    ...
    return no_diacritics_str

def gen_mailnickname(student_group):
	student_group = convert_to_identifier(student_group)
	student_group = re.sub(r"[^a-z0-9]", " ", student_group)
...
def create_m365_group(student_group,template):
	...
	m365_group = frappe.get_doc({
		...
		"enable":1,
		"template": template,
		"mailnickname": gen_mailnickname(student_group)
	})
	m365_group.insert()
